...
class CO_molecule:
 ...
 def v_P(self): # return potential energy of the bond for the vibration motion
     return 1/2*k_bond*(mag(self.bond.axis)-d)*(mag(self.bond.axis)-d)
 def v_K(self): # return kinetic energy of the vibration motion
     return 1/2*self.C.m*dot(self.C.v-self.com_v(),norm(self.bond.axis))**2+1/2*self.O.m*dot(self.O.v-self.com_v(),norm(self.bond.axis))**2
 def r_K(self): # return kinetic energy of the rotational motion 
     return 1/2*self.C.m*mag(cross(self.C.v-self.com_v(),norm(self.bond.axis)))**2+1/2*self.O.m*mag(cross(self.O.v-self.com_v(),norm(self.bond.axis)))**2
 def com_K(self): #return kinetic energy of the translational motion of the center of mass
     return 1/2*mag(self.O.v*self.O.m+self.C.v*self.C.m)*mag(self.O.v*self.O.m+self.C.v*self.C.m)/(self.O.m+self.C.m)
def collision(a1, a2):
