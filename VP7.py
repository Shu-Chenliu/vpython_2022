...
def solve_laplacian(u, u_cond, h, Niter=5000):
 ...
 for i in range(Niter):
     V[u_cond] = u[u_cond]
     V[1:-1, 1:-1] = (1/4)*(V[0:-2,1:-1]+V[1:-1,2:]+V[1:-1,0:-2]+V[2:,1:-1]) # replace this 0 by your Laplacian Solver
 return V
def get_field(V, h):
 Ex, Ey = gradient(V)
 Ex, Ey = -Ex/h, -Ey/h
...
flux=h*(sum(Ex[int(N/2)+int(L/h/2.0)+2,int(N/2)-int(d/h/2.0)-2:int(N/2)-int(d/h/2.0)+2])-sum(Ex[int(N/2)-int(L/h/2.0)-2,int(N/2)-int(d/h/2.0)-2:int(N/2)-int(d/h/2.0)+2]))
flux+=h*(sum(Ey[int(N/2)-int(L/h/2.0)-2:int(N/2)+int(L/h/2.0)+2,int(N/2)-int(d/h/2.0)-2])-sum(Ey[int(N/2)-int(L/h/2.0)-2:int(N/2)+int(L/h/2.0)+2,int(N/2)-int(d/h/2.0)+2]))
Q=flux*epsilon
C_nonideal=Q/V0
C_ideal=epsilon*L/d
print("Q=",Q)
...
if C_nonideal>C_ideal:
    print("C_nonideal>C_ideal")
